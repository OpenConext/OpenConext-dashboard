...
export default {
  ...
  description: function (policy) {
    ...
    const teamMembership =
      teamMembershipAttr.length > 0
        ? ' he/she is a member of the team ' +
          teamMembershipAttr
            .map((attr) => {
    ...
    const and =
      teamMembershipAttr.length === 0 || teamMembershipAttr.length === attrs.length
        ? ''
        : policy.allAttributesMustMatch
        ? ' and'
        : ' or'
    ...
    const description =
      'A user' + idps + ' is ' + only + ' allowed to access ' + sp + ' when' + teamMembership + and + ' ' + attributes
