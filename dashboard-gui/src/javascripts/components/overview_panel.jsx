...
class OverviewPanel extends React.Component {
  ...
  renderStrongAuthenticationContent() {
    ...
    const idpLoaLevelConfigured = (currentUser.getCurrentIdp().stepupEntities || []).some(
      (e) => e.name === app.spEntityId
    )
    if (app.strongAuthentication || idpLoaLevelConfigured) {
      return (
        <div>
          <i className="fa fa-lock"></i> <h2>{I18n.t('overview_panel.supports_ssa')}</h2>
        </div>
      )
    }
    ...
    if (this.props.app.interfedSource !== 'SURFconext') {
      return (
        <div className="mod-interfed">
          <h3>
            {I18n.t('overview_panel.interfed_source')} {this.props.app.interfedSource}
          </h3>
  ...
  renderEntityCategory(field) {
    ...
    if (entityCategory) {
      return (
        <li>
          <a href={entityCategory} target="_blank" rel="noopener noreferrer">
            {I18n.t(`overview_panel.entity_category.${entityCategory.replace(/\./g, '')}`)}
          </a>
        </li>
      )
    }
        ...
        <div className="mod-title">
          <h3
            dangerouslySetInnerHTML={{
              __html: I18n.t('overview_panel.wiki_info_html', { link: this.props.app.wikiUrl }),
            }}
          />
        </div>
      )
  ...
  renderAansluitOvereenkomstRefused() {
    ...
    return (
      <div className="mod-description">
        ...
        <p
          dangerouslySetInnerHTML={{
            __html: I18n.t('overview_panel.aansluitovereenkomstRefused', { organisation: this.props.app.organisation }),
          }}
        />
      </div>
    )
  ...
  renderPrivacyInformation() {
    ...
    return (
      <div className="mod-description">
        ...
        <p
          dangerouslySetInnerHTML={{
            __html: I18n.t('overview_panel.privacyInformationInfo', { organisation: this.props.app.organisation }),
          }}
        />
      </div>
    )
  ...
  renderSURFSecureId = () => {
    ...
    const loa = idpLoaLevelConfigured
      ? currentUser.getCurrentIdp().stepupEntities.find((e) => e.name === app.spEntityId).level
      : app.minimalLoaLevel
    ...
    if (app.strongAuthentication || idpLoaLevelConfigured) {
      return (
        <div className="mod-description">
          ...
          <p dangerouslySetInnerHTML={{ __html: I18n.t(`overview_panel.${msg}`, { minimalLoaLevel: loaStripped }) }} />
        </div>
      )
    }
    return null
      ...
      return (
        <div className="mod-description">
          ...
          <h3>
            ...
            <span
              dangerouslySetInnerHTML={{
                __html: I18n.t(`overview_panel.contractualBase.${this.props.app.contractualBase.toLowerCase()}`, {
                  organisation: this.props.app.organisation,
                }),
              }}
            />
  ...
  renderHasConnection() {
    ...
    if (currentUser.dashboardAdmin && !conflictingJiraIssue) {
      disconnect = (
        <p>
          <Link to={`/apps/${app.id}/${app.entityType}/how_to_connect`}>{I18n.t('overview_panel.disconnect')}</Link>
        </p>
      )
    }
  ...
  renderNoConnection() {
    ...
    if (currentUser.dashboardAdmin && !conflictingJiraIssue) {
      connect = (
        <p>
          <Link to={`/apps/${app.id}/${app.entityType}/how_to_connect`}>{I18n.t('overview_panel.how_to_connect')}</Link>
        </p>
      )
    }
  ...
  render() {
    ...
    const { currentUser } = this.context
    const entityID =
      app.entityType === 'oidc10_rp' ? I18n.t('overview_panel.rpClientID') : I18n.t('overview_panel.entityID')
    return (
      <div className="l-middle-app-detail">
        {currentUser.guest && (
          <div className="requires-login" onClick={this.login}>
