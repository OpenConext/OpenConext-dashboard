...
<div style="display: flex;align-items: center; margin-bottom: 5px">
  ...
  aggregatedOptions = (data, yValues, includeUniques, guest) => {
    const series = [{ name: I18n.t('chart.userCount'), color: '#D4AF37', data: data.map((p) => p.count_user_id) }]
  ...
  renderChart = (
    ...
    const yValues = aggregate
      ? userCount.map((p) => this.renderYvalue(p, identityProvidersDict, serviceProvidersDict))
      : []

    const options = aggregate
      ? this.aggregatedOptions(data, yValues, includeUniques, guest)
    ...
    return (
      <section className="chart">
        ...
        <HighChartContainer
          highcharts={aggregate ? HighChart : HighStock}
          constructorType={aggregate ? 'chart' : 'stockChart'}
          options={options}
        />
        {!aggregate && !this.props.noTimeFrame && (
          <section className="navigate">
            <span onClick={this.props.goLeft}>
              <i className="fa fa-arrow-left"></i>
            </span>
            <span onClick={this.props.goRight}>
              <i className={`fa fa-arrow-right ${rightClassName}`}></i>
            </span>
          </section>
        )}
      </section>
