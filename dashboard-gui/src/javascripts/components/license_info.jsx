...
class LicenseInfo extends React.Component {
  ...
  renderHasLicense() {
    const licenseStatus =
      this.props.app.licenseStatus === 'HAS_LICENSE_SURFMARKET'
        ? I18n.t('license_info.has_license_surfmarket')
        : I18n.t('license_info.has_license_sp')
    const license = this.props.app.license
    const licenseInfo =
      license && license.endDate
        ? I18n.t('license_info.valid', { date: I18n.strftime(new Date(license.endDate), '%-d %B %Y') })
        : ''
    return (
  ...
  renderNoLicense() {
    ...
    if (this.props.showLinks) {
      link = (
        <p>
          <Link to={`/apps/${app.id}/${app.entityType}/license_info`}>{I18n.t('license_info.license_info')}</Link>
        </p>
      )
    }
    return (
  ...
  renderUnknownLicense() {
    ...
    if (this.props.showLinks) {
      link = (
        <p>
          <Link to={`/apps/${app.id}/${app.entityType}/license_info`}>
            {I18n.t('license_info.license_unknown_info')}
          </Link>
        </p>
      )
    }
