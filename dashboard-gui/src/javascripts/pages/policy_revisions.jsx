...
class PolicyRevisions extends React.Component {
      ...
      <div className="form-element" key={index}>
        <fieldset className={classNameStatus}>
          <div className="l-grid">
            <div className="l-col-10">{this.renderRevisionMetadata(revision)}</div>
            <div className="l-col-2 no-gutter text-right">
              <a
                className={'c-button white compare ' + linkClassName}
                href="/compare"
                onClick={this.handleCompare(revision)}
              >
                &lt; &gt;
              </a>
            </div>
  ...
  renderTopDiff(prev, curr) {
    const translationKey =
      prev.revisionNbr !== undefined && prev.revisionNbr !== curr.revisionNbr
    ...
    return <div className="top-diff" dangerouslySetInnerHTML={{ __html: topDiffHtml }} />
  }
  ...
  renderAboutPage() {
    return (
      <div className="mod-policy-revisions-about">
        {I18n.locale === 'en' ? (
          <PolicyRevisionsHelpEn />
        ) : I18n.locale === 'pt' ? (
          <PolicyRevisionsHelpPt />
        ) : (
          <PolicyRevisionsHelpNl />
        )}
      </div>
    )
  }
      ...
      <div className="l-grid main">
        <div className="l-col-6">
          <div className="mod-policy-revisions">
            ...
            <form>{this.renderRevisions()}</form>
          </div>
        </div>
        <div className="l-col-6">{this.renderComparePanel()}</div>
      </div>
    )
  }
}
