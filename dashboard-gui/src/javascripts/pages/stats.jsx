...
class Stats extends React.Component {
  ...
  componentDidMount() {
    ...
    Promise.all([
      loginTimeFrame(from.unix(), to.unix(), scale, sp === this.allServiceProviderOption.value ? undefined : sp),
    ...
    if (displayDetailPerSP) {
      loginAggregated(getPeriod(from, scale), spEntityId).then((res) => {
        if (isEmpty(res)) {
          ...
          const uniqueOnes = res
            .filter((p) => p.distinct_count_user_id)
            .reduce((acc, p) => {
      ...
      uniqueLoginCount(from.unix(), to.unix(), spEntityId).then((res) =>
        this.setState({
          data: res,
          loaded: true,
        })
  ...
  goRight = (e) => {
    ...
    const from = moment(this.state.from).add(1, scale)
    const to = moment(this.state.to).add(1, scale)
    const tomorrowMidnight = moment().add(1, 'day').startOf('day').add(-3, 'minutes')
  ...
  onChangeServiceProvider = (val) => {
    ...
    this.setState(
      {
        data: [],
        sp: val ? val : this.allServiceProviderOption.value,
        ...additionalState,
      },
      this.refresh
    )
  ...
  renderYearPicker = (date, maxYear, onChange) => {
    ...
    return (
      <SelectWrapper
        ...
        options={Array.from(new Array(1 + maxYear - 2011), (x, i) => (i + 2011).toString(10)).map((m) => ({
          display: m,
          value: m,
        }))}
        multiple={false}
      ...
      return (
        <div className="group-dates">
          <SelectWrapper
            ...
            options={Array.from(new Array(4), (x, i) => 'Q' + (i + 1).toString(10)).map((m) => ({
              display: m,
              value: m,
            }))}
            multiple={false}
  ...
  renderPeriod = (scale, from, to, toEnabled, spSelected, className = '') => {
    return (
      <fieldset className={className}>
        ...
        <SelectWrapper
          defaultValue={scale}
          options={this.getScalesForPeriod(toEnabled, spSelected).map((scale) => ({
            display: I18n.t(`stats.scale.${scale}`),
            value: scale,
          }))}
          multiple={false}
        ...
        {this.renderDatePicker(
          ...
          from,
          this.onChangeFrom,
          moment(to).subtract(1, 'day'),
          getDateTimeFormat(scale, !toEnabled),
          'from-date',
          false
        )}
        {toEnabled && (
