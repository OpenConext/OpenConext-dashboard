...
class PolicyDetail extends React.Component {
  ...
  componentDidMount() {
    ...
    Promise.all(promises).then((res) => {
      this.setState({
        ...
        connectedServiceProviders: res[1].payload.filter(
          (sp) => !sp.isResourceServer && sp.entityType !== 'single_tenant_template'
        ),
      ...
      return (
        <div className="l-main">
          ...
          <div className="l-grid">
            ...
            <div className="l-col-6 no-gutter">{this.renderHelp()}</div>
          </div>
        </div>
      )
    }
        ...
        <fieldset className="success">
          <div className="l-grid">
            <div className="l-col-4">
              ...
              <div
                id="ios_checkbox"
                className={classNameSelected + ' ios-ui-select'}
  ...
  renderServiceProvider(policy) {
    ...
    const serviceProviders = (
      scopedSPs ? this.state.institutionServiceProviders : this.state.connectedServiceProviders
    ).map((sp) => {
  ...
  renderScopedWarning(scopedSPs) {
    if (scopedSPs) {
      return (
        <em className="note">
          <sup>*</sup>
          {I18n.t('policy_detail.spScopeInfo')}{' '}
        </em>
      )
    }
  ...
  renderRule(value, selected) {
    ...
    return (
      <li key={value}>
        <a href="/rule" className={className} onClick={this.handleChooseRule(value)}>
          {value}
        </a>
      </li>
    )
      ...
      <div className="form-element">
        <fieldset className="success">
          ...
          <label htmlFor="isActive">
            {I18n.t('policy_detail.isActiveDescription')}
            <a
              className="help-link"
              target="_blank"
          ...
          <em className="note">
            <sup>*</sup>
            {I18n.t('policy_detail.isActiveInfo')}{' '}
          </em>
        </fieldset>
      </div>
    )
        ...
        <fieldset>
          <div className="l-grid">
            <div className="l-col-3">
              <a className={'c-button ' + classNameSubmit} href="/submit" onClick={this.submitForm}>
                {I18n.t('policy_detail.submit')}
              </a>
            </div>
            <div className="l-col-3">
              <a className="n-button" href="/cancel" onClick={this.cancelForm}>
                {I18n.t('policy_detail.cancel')}
              </a>
            </div>
          </div>
        </fieldset>
  ...
  isValidPolicy() {
    ...
    const inValid =
      isEmpty(policy.name) ||
      isEmpty(description) ||
      isEmpty(policy.serviceProviderId) ||
  ...
  submitForm = (e) => {
    ...
    const policyEnforcementDecisionRequired = this.findServiceProvider(
