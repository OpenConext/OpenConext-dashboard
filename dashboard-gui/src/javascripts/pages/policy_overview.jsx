...
class PolicyOverview extends React.Component {
      ...
      <div className="l-main">
        ...
        <div className="mod-policy-list">
          <table>
            ...
            <tbody>
              {sort(filteredPolicies, this.state.sortAttribute, this.state.sortAscending).map(
                this.renderPolicy.bind(this)
              )}
            </tbody>
          </table>
        </div>
  ...
  renderHeader() {
    ...
    return this.isAllowedToMaintainPolicies(currentUser) ? (
      <div className="l-grid">
        <div className="l-col-8">{search}</div>
        <div className="l-col-4 text-right no-gutter">
          ...
          <a
            href="https://support.surfconext.nl/dashboard-help-pdp"
            target="_blank"
            className="t-button policy-button how-to"
      ...
      <div className="l-grid">
        <div className="l-col-12 no-gutter">{search}</div>
      </div>
    )
  }
  ...
  renderPolicy(policy, i) {
    ...
    return (
      <tr key={i}>
        ...
        <td>
          <input type="checkbox" defaultChecked={policy.active} disabled={true} />
        </td>
  ...
  renderRevisionsLink(policy) {
    ...
    return <Link to={`/policies/${policy.id}/revisions`}>{numberOfRevisions}</Link>
  }
    ...
    if (policy.actionsAllowed) {
      return (
        <div className="controls">
          ...
          <a
            href="/delete"
            data-tooltip={I18n.t('policies.delete')}
