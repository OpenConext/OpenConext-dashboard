...
class AppOverview extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      idpDisableConsent: [],
      loading: true,
      entityCategoriesFacetSelector: false,
    }
    if (store.appId) {
  ...
  componentDidMount() {
    ...
    Promise.all([getApps(), disableConsent()]).then((data) => {
    ...
    return (
      <tr>
        <td className="empty" colSpan="4">
          {I18n.t('apps.overview.no_results')}
        </td>
      </tr>
    )
  }
  ...
  renderProcessing() {
    return (
      <tr>
        <td className="processing_results" colSpan="4">
          {I18n.t('apps.overview.processing_results')}
        </td>
      </tr>
    )
  }
  ...
  renderApp(app, index) {
    ...
    return (
      ...
      >
        <td>
          <Link to={`/apps/${app.id}/${app.entityType}/overview`} onClick={(e) => this.handleShowAppDetail(e, app)}>
            {app.name}
          </Link>
        </td>
        ...
        <td className="right">{connect && this.renderConnectButton(app)}</td>
      </tr>
    )
  }
  ...
  renderLicenseNeeded(app) {
    return (
      <td className={`${this.licenseStatusClassName(app)}`}>
        {I18n.t('facets.static.license.' + app.licenseStatus.toLowerCase())}
      </td>
    )
  }
    ...
    if (!app.connected) {
      return (
        <Link
          ...
          onClick={(e) => this.handleConnectApp(e, app)}
        >
          {I18n.t('apps.overview.connect_button')}
        </Link>
      )
    }
    return null
    ...
    facets.forEach((facet) => {
      switch (facet.searchValue) {
        ...
        case 'entity_category':
          filter(facet, (app, facetValue) => {
            return (
              app.entityCategories1 === facetValue.searchValue ||
  ...
  filterBySearchQuery(app) {
    const searchString = this.state.search.toLowerCase()
    return (
      Object.values(app.descriptions)
        .concat(Object.values(app.names))
        .some((name) => name.toLowerCase().indexOf(searchString) > -1) ||
    ...
    let results = [
      ...
      },
      {
        ...
        filterApp: function (app) {
          ...
          const { entityCategoriesFacetSelector } = this.state
          ...
          return (
            <div className="entity-categories-facet-selector">
              ...
              <i
                ...
                data-for="entity-categories-facet-selector-tooltip"
                data-tip
              ></i>
              <ReactTooltip
        ...
        <section className="container">
          ...
          {rangeWithDots.map((nbr, index) =>
            typeof nbr === 'string' || nbr instanceof String ? (
              ...
              <span key={index} onClick={this.changePage.bind(this, nbr)}>
                {nbr}
              </span>
            )
          )}
  ...
  render() {
    ...
    if (currentUser.dashboardAdmin && currentUser.getCurrentIdp().institutionId) {
      connect = <th className="percent_10 right">{I18n.t('apps.overview.connect')}</th>
    }
        ...
        <div className="l-right">
          ...
          <div className="mod-app-list">
            <table>
              ...
              <tbody>
                {sortedApps.length > 0
                  ? sortedApps.map((app, index) => this.renderApp(app, index))
                  : sortedApps.length === this.state.apps || loading
