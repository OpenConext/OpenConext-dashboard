...
class History extends React.Component {
    ...
    return (
      <section className="pagination">
        <section className="container">
          {nbrPages > 1 && page !== 1 && <i className="fa fa-arrow-left" onClick={this.changePage(page - 1)}></i>}
          {rangeWithDots.map((nbr, index) =>
            typeof nbr === 'string' || nbr instanceof String ? (
              ...
              <span key={index} onClick={this.changePage(nbr)}>
                {nbr}
              </span>
            )
          )}
    ...
    return (
      <section className="filters">
        <div className="header">
          ...
          <a href="refresh" className="refresh c-button" onClick={this.refresh}>
            {I18n.t('facets.refresh')}
          </a>
  ...
  renderResolution = (action) => {
    if (action.resolution) {
      ...
      return (
        <span className="actionResolution">
          {I18n.t(`history.resolution.${transKey}`)}
  ...
  renderAction = (action, index) => {
    ...
    return (
      <tr key={action.jiraKey} className={action.status ? action.status.toLowerCase().replace(' ', '-') : ''}>
        ...
        <td>
          {renderAction ? (
            <a href="/send" className={'t-button save'} onClick={this.viewInvitation(action)}>
              {I18n.t('history.viewInvitation')}
            </a>
          ) : renderResend ? (
            <a href="/resend" className={'t-button save'} onClick={this.resendInvitation(action)}>
              {I18n.t('history.resendInvitation')}
            </a>
          ) : (
            this.renderResolution(action)
          )}
        </td>
      </tr>
    )
  ...
  render() {
    ...
    const moreAwaitingTickets =
      awaitingInputTickets > actions.filter((action) => action.status === 'Awaiting Input').length
    return (
      <div>
        <Flash />
        <ConfirmationDialog
          ...
          <div className="table_wrapper">
            ...
            {loaded && sortedActions.length > 0 && (
              <table>
                ...
                <tbody>{sortedActions.map((action, index) => this.renderAction(action, index))}</tbody>
              </table>
            )}
            {loaded && sortedActions.length === 0 && (
              <div>
