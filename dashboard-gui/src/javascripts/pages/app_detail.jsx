...
class AppDetail extends React.Component {
  ...
  componentDidMount() {
    ...
    Promise.all([getApp(params.id, params.type), disableConsent()]).then((data) => {
            ...
            if (nonRejected.length > 0) {
              ...
              if (!action.rejected) {
                if (params.jiraKey && action.status !== 'Awaiting Input') {
                  ...
                  setFlash(
                    I18n.t('apps.detail.inviteAlreadyProcessed', {
  ...
  renderNavItem(panelKey) {
    ...
    return (
      <li key={panelKey}>
        <Link
          to={`/apps/${this.props.match.params.id}/${this.props.match.params.type}/${panelKey}`}
  ...
  renderActivePanel() {
    ...
    const { currentUser } = this.context
    const { app, institutions, idpDisableConsent, jiraKey, inviteAction, conflictingJiraIssue, resourceServers } =
      this.state
    let panel = this.panelMap[activePanel]
    if (
      !panel ||
      (activePanel === 'how_to_connect' && !(currentUser.dashboardAdmin && currentUser.getCurrentIdp().id))
    ) {
